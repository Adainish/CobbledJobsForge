...
import io.github.adainish.cobbledjobsforge.obj.configurabledata.ActionKey;
import io.github.adainish.cobbledjobsforge.obj.configurabledata.ConfigurableJob;
import io.github.adainish.cobbledjobsforge.obj.configurabledata.ConfigurableLevel;
import io.github.adainish.cobbledjobsforge.obj.configurabledata.JobType;
import io.github.adainish.cobbledjobsforge.storage.PlayerStorage;
import io.github.adainish.cobbledjobsforge.util.EconomyUtil;
...
public class Player
{
    ...
    public void updateCache() {
        CobbledJobsForge.dataWrapper.playerCache.put(uuid, this);
    }
    ...
    public void syncWithJobManager() {
        if (jobs.isEmpty()) {
            CobbledJobsForge.jobsConfig.jobManager.configurableJobs.forEach((s, configurableJob) -> {
                Job job = new Job();
                job.jobName = s;
                jobs.put(s, job);
            });
            ...
            jobs.forEach((s, job) -> {
                if (CobbledJobsForge.jobsConfig.jobManager.configurableJobs.containsKey(s))
                    return;
                job.hasJob = false;
            });
            CobbledJobsForge.jobsConfig.jobManager.configurableJobs.forEach((s, configurableJob) -> {
    ...
    public List<Button> sortedJobsButtons() {
        ...
        List<ConfigurableJob> configurableJobs = new ArrayList<>(CobbledJobsForge.jobsConfig.jobManager.configurableJobs.values());
